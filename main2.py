...
class SolarSystem:
    ...
    """def draw_skybox(self):
        ...
        glEnable(GL_TEXTURE_2D)
        glBindTexture(GL_TEXTURE_2D, self.skybox_texture)
        glDepthMask(GL_FALSE)  # Désactive le depth buffer pour la skybox

        # Dessine un cube géant (meilleur pour les skybox que la sphère)
        size = 1000
        glBegin(GL_QUADS)
    ...
    # Nouveau code encore une amelioration
    """def draw_skybox(self):
        ...
        # Dessiner un cube au lieu d'une sphère
        size = 500
        glBegin(GL_QUADS)
        # Face avant
        glTexCoord2f(0, 0); glVertex3f(-size, -size, -size)
    ...
    def draw_skybox(self):
        ...
        # Activer la transparence et désactiver l'écriture dans le depth buffer
        glEnable(GL_BLEND)
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
        glDepthMask(GL_FALSE)  # Empêche la skybox d'écraser le depth buffer
        ...
        glDepthMask(GL_TRUE)  # Réactiver le depth buffer
        glDisable(GL_BLEND)
        glDisable(GL_TEXTURE_2D)
        glEnable(GL_LIGHTING)
...
# Amelioration initialize
"""def initialize():
    glEnable(GL_DEPTH_TEST)
    glDepthFunc(GL_LEQUAL)  # Meilleure gestion de la profondeur
    glEnable(GL_LIGHTING)
    glEnable(GL_LIGHT0)
    glEnable(GL_COLOR_MATERIAL)
    glClearColor(0.0, 0.0, 0.0, 1.0)  # Alpha à 1
